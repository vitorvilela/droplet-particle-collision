...
print('\nFactorial design for dimensionless lamella area at t*~1 (0.918)')
...
print('\nChosing the power of the test = 0.9, which leads to a significance level (alpha) = 0.1')

alpha = 0.1
print('\nalpha/2')
print(alpha/2)

dof = (len(dataset_list)-2)
print('\ndof')
print(dof)
...
print('\n\nThe two-sample t-test for FACTOR A (REYNOLDS)')

mean_high_level = np.mean(list_a+list_ab)
print('\nmean_high_level')
print(mean_high_level)
mean_low_level = np.mean(list_o+list_b)
print('\nmean_low_level')
print(mean_low_level)

n_high_level = len(list_a+list_ab)
print('\nn_high_level')
print(n_high_level)
n_low_level = len(list_o+list_b)
print('\nn_low_level')
print(n_low_level)

var_high_level = np.var(list_a+list_ab, ddof=1)
print('\nvar_high_level')
print(var_high_level)
var_low_level = np.var(list_o+list_b, ddof=1)
print('\nvar_low_level')
print(var_low_level)

print('\nCHECK: Because the sample standard deviations are reasonably similar, it is not unreasonable to con- clude that the population standard deviations (or variances) are equal. \nSo it is possible to proceed with the following formulations...')

estimated_common_variance = ( (n_high_level-1)*var_high_level + (n_low_level-1)*var_low_level ) / dof
print('\nestimated_common_variance')
print(estimated_common_variance)

standard_error = np.sqrt( estimated_common_variance*(1/n_high_level + 1/n_low_level) )
print('\nstandard_error (of the difference in means)')
print(standard_error)

t0 = (mean_high_level - mean_low_level ) / standard_error
print('\n|t0|')
print(np.abs(t0))

print('\nTo determine whether to reject H0 :mu1 = mu2, we would compare t0 to the t distribution with n1 + n2 - 2 degrees of freedom')

print('\nIf |t0| > t_alpha/2,n1+n2-2, where t_alpha/2,n1+n2-2 is the upper alpha/2 percentage point of the t distribution with n1 + n2 -1 2 degrees of freedom, we would reject H0 (mu1 = m2) and conclude that the mean differ')

if (np.abs(t0) > t):
  print('\nmu1 != mu2 .:. factor A (Reynolds) affects the result (interfacial area)')
else:
  print('\nmu1 == mu2 .:. factor A (Reynolds) does not affect the result (interfacial area)')




print('\n\nThe two-sample t-test for FACTOR B (WEBER)')

mean_high_level = np.mean(list_b+list_ab)
print('\nmean_high_level')
print(mean_high_level)
mean_low_level = np.mean(list_o+list_a)
print('\nmean_low_level')
print(mean_low_level)

n_high_level = len(list_b+list_ab)
print('\nn_high_level')
print(n_high_level)
n_low_level = len(list_o+list_a)
print('\nn_low_level')
print(n_low_level)

var_high_level = np.var(list_b+list_ab, ddof=1)
print('\nvar_high_level')
print(var_high_level)
var_low_level = np.var(list_o+list_a, ddof=1)
print('\nvar_low_level')
print(var_low_level)
...
if (np.abs(t0) > t):
  print('\nmu1 != mu2 .:. factor B (Weber) affects the result (interfacial area)')
else:
  print('\nmu1 == mu2 .:. factor B (Weber) does not affect the result (interfacial area)')
