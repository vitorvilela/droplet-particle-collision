...
rescaledX = train_input
  
#scaler_filename = 'amirfazli_degree3_scaler.sav'
#dump(scaler, scaler_filename, protocol=2)
...
##m = dict(zip(names[1:4], reg.coef_))




model_filename = 'linear_degree2_model.sav'
dump(model, model_filename, protocol=2)
...
#scaler_mean = dict(zip(polynomial_features.get_feature_names(), scaler.mean_))
#scaler_std = dict(zip(polynomial_features.get_feature_names(), scaler.scale_))
...
m_original = dict(zip(polynomial_features.get_feature_names(), reg.coef_))
#m_original = dict(zip(names[1:4], reg.coef_))

#features_list = polynomial_features.get_feature_names()


## https://stats.stackexchange.com/questions/74622/converting-standardized-betas-back-to-original-variables
#sum_coeffs = 0.
#for f in features_list:
  #m_original[f] = m_original[f]/scaler_std[f] #+scaler_mean[f]
  #sum_coeffs += m_original[f]*scaler_mean[f]/scaler_std[f]
...
#intercept_original = reg.intercept_-sum_coeffs
intercept_original = reg.intercept_
