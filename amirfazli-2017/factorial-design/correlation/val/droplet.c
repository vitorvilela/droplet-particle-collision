...
int main() {
  ...
  const double ef[replicates] = {1.};
  const double reynolds[variations] = {1.8e3, 3.4e3, 5.1e3, 7.9e3};
  const double weber[variations] = {1.6e2, 4.4e2, 6.2e2, 8.9e3};
...
event factorial (t=0; t<duration; t+=checkat) {
  ...
  if (pid() == 0) {
    ...
    if (i == 0)
      factorialFile = fopen ("data.csv", "a+");
    
    if ( t == 0 || fabs(t-0.2*Dd/Vd) < checkat || fabs(t-0.5*Dd/Vd) < checkat || fabs(t-0.8*Dd/Vd) < checkat || fabs(t-1.0*Dd/Vd) < checkat || fabs(t-1.2*Dd/Vd) < checkat || fabs(t-1.5*Dd/Vd) < checkat ) {
...
event snapshot (t=0; t<duration; t+=checkat) {
  ...
  if ( t == 0 || fabs(t-0.5*Dd/Vd) < checkat || fabs(t-1.0*Dd/Vd) < checkat || fabs(t-1.5*Dd/Vd) < checkat ) {
    ...
    // TODO: Check if you created ./dump and ./figs folders
