...
int main() {
  ...
  printf ("\nCase: %d\tRe: %d\tWe: %d\nCoarser and finer grids (micrometers): %f\t%f\nDd/DeltaMIN: %f\n", CASE, (int)(Re), (int)(We), (DOMAIN_LENGTH/N)*SPATIALSCALER, (DOMAIN_LENGTH/M)*SPATIALSCALER, Dd/(DOMAIN_LENGTH/M));
...
event snapshot (t=0; t+=print2Dat; t<duration) {
  ...
  sprintf(ppm_name_vof, "vof-%2.1f.ppm", t*TIMESCALER); // naming as miliseconds
  
  char ppm_name_u[80];
  sprintf(ppm_name_u, "u-%2.1f.ppm", t*TIMESCALER);   
  
  char ppm_name_lambda2[80];
  sprintf(ppm_name_lambda2, "lambda2-%2.1f.ppm", t*TIMESCALER); 
  
  char ppm_name_level[80]; 
  sprintf(ppm_name_level, "level-%2.1f.ppm", t*TIMESCALER);
...
event snapshot3D (t=0; t+=print3Dat; t<duration) {
  ...
  char name[80];
  sprintf (name, "dump-%2.1f", t*TIMESCALER); // naming as miliseconds
  dump (file = name);  
