...
event init(i = 0) {
  ...
  const double dump_number[numberOfFiles] = {0.000, 2.000, 4.000, 6.000, 8.000, 10.000, 12.000, 14.000, 16.000, 18.000, 20.000};
  char dump_name[20], image_name[20];
  
  for (int i=0; i<numberOfFiles; i++) {    
     
    sprintf(dump_name, "dump-%2.3f", dump_number[i]);
    printf("%s\n", dump_name);
...
//     boundary((scalar *){u});
    ...
    /* tx, ty: shifts the camera center point.
     ...
     view (ty = -.15, fov = 15, width = mesh, height = mesh, camera = "iso", bg = {1, 1, 1});
    ...
    sprintf (image_name, "iso-%2.3f.png", dump_number[i]);
    save (image_name);
        
        
    
    clear ();    
    view (ty = -.5, fov = 25, width = mesh, height = mesh, camera = "front", bg = {1, 1, 1});
    //view (ty = -0.0, fov = 45, width = mesh, height = mesh, camera = "front", bg = {1, 1, 1});
    box (notics=true);
    draw_vof ("f", color = "f", min = 0, max = 10);
    draw_vof ("particle", color = "f", min = -10, max = 0);
    
    sprintf (image_name, "front-%2.3f.png", dump_number[i]);
    save (image_name);
    
    
//     clear ();    
//     view (ty = -0.01, fov = 20, width = mesh, height = mesh, camera = "bottom", bg = {1, 1, 1});
//     box (notics=true);
//     draw_vof ("f", color = "f", min = 0, max = 10);
// //    draw_vof ("particle", color = "f", min = -10, max = 0);
//     
//     sprintf (image_name, "bottom-%i.png", dump_number[i]);
//     save (image_name);
